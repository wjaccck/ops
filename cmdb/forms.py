...
class MachineForm(forms.ModelForm):
    ...
    vcpu_number=forms.IntegerField(label='vcpu_number',required=False)
    cpu_core=forms.IntegerField(label='cpu_core',required=False)
    hostname=forms.CharField(label='主机名', max_length=100,required=False, widget=forms.TextInput({'class': 'form-control'}))
    memory=forms.IntegerField(label='内存',required=False)
